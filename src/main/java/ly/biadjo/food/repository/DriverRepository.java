...
public interface DriverRepository extends JpaRepository<Driver, Long>, JpaSpecificationExecutor<Driver> {
    ...
    @Query(value = "select driver from Driver driver left join fetch driver.zone", countQuery = "select count(driver) from Driver driver")
    Page<Driver> findAllWithToOneRelationships(Pageable pageable);

    @Query("select driver from Driver driver left join fetch driver.zone")
    List<Driver> findAllWithToOneRelationships();

    @Query("select driver from Driver driver left join fetch driver.zone where driver.id =:id")
