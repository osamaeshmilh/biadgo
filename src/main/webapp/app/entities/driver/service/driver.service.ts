...
type RestOf<T extends IDriver | NewDriver> = Omit<T, 'lastLocationDateTime'> & {
...
export class DriverService {
  ...
  protected convertDateFromClient<T extends IDriver | NewDriver | PartialUpdateDriver>(driver: T): RestOf<T> {
    return {
      ...driver,
      lastLocationDateTime: driver.lastLocationDateTime?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restDriver: RestDriver): IDriver {
    return {
      ...restDriver,
      lastLocationDateTime: restDriver.lastLocationDateTime ? dayjs(restDriver.lastLocationDateTime) : undefined,
    };
  }
